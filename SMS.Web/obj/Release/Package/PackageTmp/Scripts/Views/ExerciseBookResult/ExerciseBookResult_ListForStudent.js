...
function FillExerciseBookResult() {
    tblExerciseBookResult = $('#tblExerciseBookResult').dataTable({
        "sAjaxSource": "../api/Student/GetStudentsForExerciseBookResultDataView",
        fnServerData: function (sUrl, aoData, fnCallback, oSettings) {
            var pageDetail = GetPageDetail(oSettings);
            oSettings.oLanguage.sSearch = "exercisebook";
            $.ajax({
                dataType: 'json',
                type: "POST",
                contentType: "application/json; charset=utf-8",
